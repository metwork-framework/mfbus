...
## How to install mfbus metwork module
...
### Optional mfbus layers

To list available (and not already installed) mfbus additional
layers, you can use the following command:

```console
# For CentOS or Fedora (see above note for other distributions)
yum list |grep 'metwork-mfbus-layer-.*:'
```
...
## How to uninstall mfbus module

To uninstall mfbus metwork module, use the following command (as `root` user):
